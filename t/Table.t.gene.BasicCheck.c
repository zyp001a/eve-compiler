...
int main(int argc, char **argv){
	...
	get_col_val_dump();
	analysis_col_typev();
	printf("gene.ColumnNumber =  %d\n", ccol);
	printf("gene.HeadStartOffset =  %ld\n", thead);
	printf("gene.DataStartOffset =  %ld\n", tstart);
	int coli;
//	printf("gene.DraftStruct.Name = \'geneLine\'\n");
//	printf("\^Include\n\^\n");
	for(coli=0; coli<ccol; coli++){
		printf("gene.DraftStruct.Definitions[%d]:C.Definition\n", coli);
		printf("gene.DraftStruct.Definitions[%d].Type.Name = \'%s\'\n", 
					 coli, TypeVNames[colstat[coli].type]);
		printf("gene.DraftStruct.Definitions[%d].Name = \'%s\'\n",
           coli, colstat[coli].name);
		if(colstat[coli].type == TWORD)
			printf("gene.DraftStruct.Definitions[%d].Type.Size = 15\n", coli);
